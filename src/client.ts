...
export interface ClientOptions {
  /**
   * Defaults to process.env['HYPEMAN_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['HYPEMAN_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['HYPEMAN_LOG'] or 'warn' if it isn't set.
   */
...
export class Hypeman {
   ...
   */
  constructor({
    baseURL = readEnv('HYPEMAN_BASE_URL'),
    apiKey = readEnv('HYPEMAN_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.HypemanError(
        "The HYPEMAN_API_KEY environment variable is missing or empty; either provide it, or instantiate the Hypeman client with an apiKey option, like new Hypeman({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Hypeman.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('HYPEMAN_LOG'), "process.env['HYPEMAN_LOG']", this) ??
      defaultLogLevel;
  ...
  static HypemanError = Errors.HypemanError;
