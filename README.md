...
```sh
npm install git+ssh://git@github.com:stainless-sdks/hypeman-typescript.git
```
...
const response: Hypeman.HealthCheckResponse = await client.health.check();
```
...
const response = await client.health.check().catch(async (err) => {
  if (err instanceof Hypeman.APIError) {
...
1. Via the `HYPEMAN_LOG` environment variable
...
const client = new Hypeman({
  logger: logger.child({ name: 'Hypeman' }),
