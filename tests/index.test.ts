...
import { APIPromise } from '@onkernel/hypeman/core/api-promise';

import util from 'node:util';
import Hypeman from '@onkernel/hypeman';
import { APIUserAbortError } from '@onkernel/hypeman';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['HYPEMAN_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Hypeman) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      const client = new Hypeman({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'HYPEMAN_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Hypeman({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['HYPEMAN_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Hypeman({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['HYPEMAN_BASE_URL'] = 'https://example.com/from_env';
      const client = new Hypeman({ apiKey: 'My API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new Hypeman({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['HYPEMAN_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Hypeman({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Hypeman({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Hypeman({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['HYPEMAN_API_KEY'] = 'My API Key';
    const client = new Hypeman();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['HYPEMAN_API_KEY'] = 'another My API Key';
    const client = new Hypeman({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Hypeman({ apiKey: 'My API Key', fetch: testFetch });
